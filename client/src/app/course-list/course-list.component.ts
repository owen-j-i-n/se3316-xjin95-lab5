...
export class CourseListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.$http.get(`${apiUrl}/get_courseList?token=${token}`).subscribe((resp) => {
  ...
  deleteCourseList(courseList_id) {
    this.$http.get(`${apiUrl}/delete_courseList?courseList_id=${courseList_id}&token=${localStorage.getItem("token")}`).subscribe((resp) => {
      if (resp["code"] == "0") {
  ...
  saveCourseList() {
    ...
    if (this.cur_modal_type == "edit") {
      ...
      this.$http.post(`${apiUrl}/edit_course_list`, edit_courseList, httpOptions).subscribe((resp) => {
        if (resp["code"] == "0") {
      ...
      this.$http.post(`${apiUrl}/add_course_list`, add_courseList, httpOptions).subscribe((resp) => {
        if (resp["code"] == "0") {
          this.CourseList_loading = false;
          this.CourseList_visible = false;
    ...
    this.resetFormData();
    this.$http.get(`${apiUrl}/get_courseList?token=${localStorage.getItem("token")}`).subscribe((resp) => {
      this.courseLists = resp["courseLists"];
      for (var course of this.courseLists) {
        course.expand = false;
      }
  ...
  NameAsyncValidator = (control: FormControl) =>
    new Observable((observer: Observer<ValidationErrors | null>) => {
      this.$http.get(`${apiUrl}/course_list_name_duplicated?name=${control.value}&courseList_id=${this.cur_modal_type == "edit" ? this.cur_editCourseList._id : ""}`).subscribe((resp) => {
        if (resp["duplicated"]) {
  ...
  saveReview() {
    ...
    this.$http.post(`${apiUrl}/add_review`, { review: this.current_Review, course_id: this.current_Review_CourseList_id, authorization: localStorage.getItem("token") }, httpOptions).subscribe((resp) => {
      if (resp["code"] == 0) {
